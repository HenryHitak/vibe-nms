...
`Discover AP Clients Now`는 백엔드에서 등록된 AP 또는 AP controller를 한 번 조회하는 수동 실행 버튼입니다. 브라우저가 Wi-Fi를 직접 스캔하지 않습니다. 결과가 0 AP로 나오면 먼저 `Device Master`에서 Access Point를 등록하고 `Device Type`을 `AP`로 설정해야 합니다. ADMIN은 선택된 AP의 `Known Client CRUD` 영역에서 해당 AP에 붙어야 하는 Client를 추가, 수정, 삭제합니다.
...
`Alert Center`의 `Mark Reviewed`는 관리자가 확인했다는 표시이며 Alert는 계속 남습니다. `Close Alert`는 Alert를 종료합니다. `Mute`는 해당 Alert type의 새 notification만 막고, Alert 기록과 상태는 그대로 유지합니다.
