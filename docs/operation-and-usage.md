...
6. Dashboard `Devices` table can multi-select devices with checkboxes; click `Selected: count` to open one compact table with all selected device information, then click `Excel` to download `selected-devices.xlsx`.
