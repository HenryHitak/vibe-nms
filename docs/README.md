...
- AP Clients: AP별 무선 Client 조회 및 모니터링 화면

AP Clients 화면은 모니터링 전용입니다. 무선 Client를 기준 데이터로 등록하거나 수정해야 하면 `Device Master`에서 `PC`, `LAPTOP`, `MOBILE`, `TABLET`, `SCANNER`, `IOT` 같은 실제 기기 타입으로 등록합니다.
